...
let lib0_diff = require("lib0/diff");
let lib0_function = require("lib0/function");
let lib0_delta = require("lib0/delta");
lib0_delta = __toESM(lib0_delta);
...
function createNodeFromLoroObj(schema, obj, mapping, onError) {
	...
	if (obj instanceof loro_crdt.LoroMap) {
...
function createLoroText(parentList, pos, nodes, mapping) {
	const obj = parentList.insertContainer(pos ?? parentList.length, new loro_crdt.LoroText()).getAttached();
...
function updateLoroText(obj, nodes, mapping) {
	...
	const { insert, remove, index } = (0, lib0_diff.simpleDiff)(str, content.map((c) => c.insert).join(""));
...
function computeChildEqualityFactor(obj, node, mapping) {
	...
	for (; left < minLength; left++) {
		...
		if (eqMappedNode(leftLoro != null && (0, loro_crdt.isContainer)(leftLoro) ? mapping.get(leftLoro.id) : void 0, leftNode)) foundMappedChild = true;
	...
	for (; left + right < minLength; right++) {
		...
		if (eqMappedNode(rightLoro != null && (0, loro_crdt.isContainer)(rightLoro) ? mapping.get(rightLoro.id) : void 0, rightNode)) foundMappedChild = true;
...
function createLoroMap(parentList, pos, node, mapping) {
	const obj = parentList.insertContainer(pos ?? parentList.length, new loro_crdt.LoroMap()).getAttached();
...
function getLoroMapAttributes(obj) {
	return obj.getOrCreateContainer(ATTRIBUTES_KEY, new loro_crdt.LoroMap());
}
...
function getLoroMapChildren(obj) {
	return obj.getOrCreateContainer(CHILDREN_KEY, new loro_crdt.LoroList());
}
function updateLoroMapChildren(obj, node, mapping) {
	...
	for (; left < minLength; left++) {
		...
		if ((0, loro_crdt.isContainer)(leftLoro) && mapping.get(leftLoro.id) !== leftNode) if (eqMappedNode(mapping.get(leftLoro.id), leftNode) || eqLoroObjNode(leftLoro, leftNode)) if (!Array.isArray(leftNode)) updateLoroMap(leftLoro, leftNode, mapping);
		else mapping.set(leftLoro.id, leftNode);
		else if (leftLoro instanceof loro_crdt.LoroText && Array.isArray(leftNode)) mapping.set(leftLoro.id, leftNode);
		else break;
	}
	for (; right + left < minLength; right++) {
		...
		if ((0, loro_crdt.isContainer)(rightLoro) && mapping.get(rightLoro.id) !== rightNode) if (eqMappedNode(mapping.get(rightLoro.id), rightNode) || eqLoroObjNode(rightLoro, rightNode)) if (!Array.isArray(rightNode)) updateLoroMap(rightLoro, rightNode, mapping);
	...
	while (loroChildLength - left - right > 0 && nodeChildLength - left - right > 0) {
		...
		if (leftLoro instanceof loro_crdt.LoroText && Array.isArray(leftNode)) {
			...
			let updateRight = rightLoro instanceof loro_crdt.LoroMap && eqNodeName(rightLoro, rightNode);
			...
			if (updateLeft) {
				...
				if ((0, loro_crdt.isContainer)(child)) mapping.delete(child.id);
	...
	if (loroChildLength === 1 && nodeChildLength === 0 && loroChildren.get(0) instanceof loro_crdt.LoroText) {
		const loroText = loroChildren.get(0);
		mapping.delete(loroText.id);
		loroText.delete(0, loroText.length);
	} else if (loroDelLength > 0) {
		loroChildren.toArray().slice(left, left + loroDelLength).filter(loro_crdt.isContainer).forEach((type) => mapping.delete(type.id));
...
function safeSetSelection(view, anchor, head) {
	...
	const selection = prosemirror_state.TextSelection.between($anchor, $head || $anchor);
	view.dispatch(view.state.tr.setSelection(selection));
}

//#endregion
//#region src/sync-plugin-key.ts
const loroSyncPluginKey = new prosemirror_state.PluginKey("loro-sync");
/**
...
function createDecorations(state, store, _plugin, createSelection, createCursor) {
	...
	if (!loroState) return prosemirror_view.DecorationSet.create(state.doc, []);
...
function absolutePositionToCursor(pmRootNode, anchor, doc, mapping) {
	...
	while (index >= 0 && childIndex < children.length) {
		...
		childIndex += 1;
		if (child instanceof loro_crdt.LoroText) {
...
function cursorToAbsolutePosition(cursor, doc, mapping) {
	...
	if (!(0, loro_crdt.isContainerId)(containerId)) return [null, void 0];
	...
	let loroNode;
	let update;
	if (container instanceof loro_crdt.LoroList) {
		...
		index = 0;
	} else if (container instanceof loro_crdt.LoroText) {
	...
	while (loroNode != null) {
		...
		if (children instanceof loro_crdt.LoroList) {
...
function findContainerLocationUncached(doc, containerId, mapping, loroDoc) {
	...
	if (foundPos == null) {
		if (loroDoc != null) {
			...
			if (container instanceof loro_crdt.LoroMap) {
...
function computeBlockPosFromLoro(blockMap, pmDoc, loroDoc, mapping) {
	...
	if (!(parentList instanceof loro_crdt.LoroList)) return null;
	const grandParent = parentList.parent();
	if (!(grandParent instanceof loro_crdt.LoroMap)) return null;
...
function findEmptyTextPosition(pmDoc, textId, mapping, loroDoc) {
	...
	if (!(text instanceof loro_crdt.LoroText)) return null;
	const parentList = text.parent();
	if (!(parentList instanceof loro_crdt.LoroList)) return null;
	const parentBlock = parentList.parent();
	if (!(parentBlock instanceof loro_crdt.LoroMap)) return null;
	...
	for (let i = 0; i < parentList.length; i++) {
		const sibling = parentList.get(i);
		if ((0, loro_crdt.isContainer)(sibling) && sibling.id === textId) {
			textIdx = i;
			break;
		}
	}
	...
	for (let i = 0; i < textIdx; i++) {
		const sibling = parentList.get(i);
		if (!(0, loro_crdt.isContainer)(sibling)) return null;
		const siblingMapped = mapping.get(sibling.id);
...
function markAncestorsDirty(doc, target, set) {
	...
	let p = c.parent();
	while (p != null) {
		if (p instanceof loro_crdt.LoroMap && p.get(NODE_NAME_KEY) != null) set.add(p.id);
		if (p instanceof loro_crdt.LoroMap || p instanceof loro_crdt.LoroList || p instanceof loro_crdt.LoroText) p = p.parent();
		else break;
	}
}
...
function applyTextDiff(tr, state, target, diff, mapping, doc, parentTouchedInBatch, dirtyAncestorBlocks, locationCache) {
	...
	if (loc != null && loc.isText) prePos = loc.pos;
		...
		if (!(text instanceof loro_crdt.LoroText)) return false;
	...
	for (const op of diff.diff) if (op.retain != null) {
		if (usedFallback && op.retain > 0) {
			...
			if (loroText instanceof loro_crdt.LoroText && loroText.length >= op.retain) {
				cursor += op.retain;
				continue;
			}
			return false;
...
function anyAncestorTouched(container, parentTouchedInBatch) {
	...
	while (node != null) {
		if (node instanceof loro_crdt.LoroMap && parentTouchedInBatch.has(node.id)) return true;
		if (node instanceof loro_crdt.LoroMap || node instanceof loro_crdt.LoroList || node instanceof loro_crdt.LoroText) node = node.parent();
		else break;
	}
	return false;
}
...
function applyListDiff(tr, state, target, diff, mapping, doc, materialisedInBatch, parentTouchedInBatch, dirtyAncestorBlocks, locationCache) {
	...
	if (!(loroList instanceof loro_crdt.LoroList)) return false;
	const parentMap = loroList.parent();
	if (!(parentMap instanceof loro_crdt.LoroMap)) return false;
	...
	for (const op of diff.diff) if (op.retain != null) for (let i = 0; i < op.retain; i++) {
		...
		for (const value of inserted) {
			if (!(0, loro_crdt.isContainer)(value)) return false;
		...
		const fragment = prosemirror_model.Fragment.from(fragments);
...
function collectContainerIds(container, set) {
	...
	if (container instanceof loro_crdt.LoroMap) for (const key of container.keys()) {
		const value = container.get(key);
		if ((0, loro_crdt.isContainer)(value)) collectContainerIds(value, set);
	}
	else if (container instanceof loro_crdt.LoroList) for (let i = 0; i < container.length; i++) {
...
function materializeInsertedContainer(schema, container, mapping) {
	if (container instanceof loro_crdt.LoroMap) return createNodeFromLoroObj(schema, container, mapping);
	if (container instanceof loro_crdt.LoroText) return createNodeFromLoroObj(schema, container, mapping);
	return null;
}
function applyMapDiff(tr, state, target, diff, mapping, doc, dirtyAncestorBlocks, locationCache) {
	...
	if (!(container instanceof loro_crdt.LoroMap)) return false;
	const parent = container.parent();
	if (!(parent instanceof loro_crdt.LoroMap)) return false;
	if (parent.get(NODE_NAME_KEY) == null) return false;
	const attrsContainer = parent.get(ATTRIBUTES_KEY);
	if (!(attrsContainer instanceof loro_crdt.LoroMap) || attrsContainer.id !== target) return false;
	...
	else if ((0, loro_crdt.isContainer)(raw)) return false;
...
*/
const isLoroText = (c) => typeof c?.id === "string" && c.id.endsWith(":Text");
const isLoroMap = (c) => typeof c?.id === "string" && c.id.endsWith(":Map");
...
function nodeToDelta(node) {
	const d = lib0_delta.create(node.type.name);
...
function deltaToPSteps(tr, d, pnode = tr.doc, currPos = { i: 0 }) {
	...
	for (const op of d.children) if (lib0_delta.$retainOp.check(op)) {
		let i = op.retain;
		while (i > 0) {
	...
	} else if (lib0_delta.$modifyOp.check(op)) {
	...
	} else if (lib0_delta.$insertOp.check(op)) {
		const newNodes = op.insert.map((ins) => deltaToPNode(ins, schema, op.format));
		tr.replace(currPos.i, currPos.i, new prosemirror_model.Slice(prosemirror_model.Fragment.from(newNodes), 0, 0));
		currPos.i += newNodes.reduce((s, c) => c.nodeSize + s, 0);
	} else if (lib0_delta.$textOp.check(op)) {
		const marks = formatToMarks(op.format, schema);
		tr.replace(currPos.i, currPos.i, new prosemirror_model.Slice(prosemirror_model.Fragment.from(schema.text(op.insert, marks)), 0, 0));
		currPos.i += op.insert.length;
	} else if (lib0_delta.$deleteOp.check(op)) {
		...
		while (remaining > 0) {
			...
			if (pc.isText) {
				...
				tr.replace(currPos.i, currPos.i + delLen, prosemirror_model.Slice.empty);
				nOffset += delLen;
				...
				tr.replace(currPos.i, currPos.i + pc.nodeSize, prosemirror_model.Slice.empty);
				currParentIndex++;
				remaining--;
			}
		}
...
function deltaToPNode(d, schema, dformat) {
	...
	const children = Array.from(d.children ?? []).flatMap((op) => {
		if (lib0_delta.$insertOp.check(op)) return op.insert.map((c) => deltaToPNode(c, schema, op.format));
		if (lib0_delta.$textOp.check(op)) return [schema.text(op.insert, formatToMarks(op.format, schema))];
		return [];
	});
...
function diffPmDocs(tr, oldDoc, newDoc) {
	...
	const diff = lib0_delta.diff(oldDelta, newDelta);
	let hasChanges = false;
	for (const op of diff.children) if (!lib0_delta.$retainOp.check(op)) {
		hasChanges = true;
		break;
	}
...
*/
function applyBlockDiff(tr, oldDoc, newDoc, diff) {
	let oldIdx = 0;
	let newIdx = 0;
	let pos = 0;
	for (const op of diff.children) if (lib0_delta.$retainOp.check(op)) for (let i = 0; i < op.retain; i++) {
		...
		tr.replace(pos, pos + oldBlock.nodeSize, new prosemirror_model.Slice(prosemirror_model.Fragment.from(newBlock), 0, 0));
		pos += newBlock.nodeSize;
		oldIdx++;
		newIdx++;
	} else if (lib0_delta.$insertOp.check(op)) {
		const insertedBlocks = [];
		for (let i = 0; i < op.insert.length; i++) insertedBlocks.push(newDoc.child(newIdx++));
		tr.replace(pos, pos, new prosemirror_model.Slice(prosemirror_model.Fragment.from(insertedBlocks), 0, 0));
		pos += insertedBlocks.reduce((s, b) => s + b.nodeSize, 0);
	} else if (lib0_delta.$deleteOp.check(op)) {
		let deleteSize = 0;
		for (let i = 0; i < op.delete; i++) deleteSize += oldDoc.child(oldIdx++).nodeSize;
		tr.replace(pos, pos + deleteSize, prosemirror_model.Slice.empty);
	}
	return tr;
}
/**
...
*/
function rebuildMappingAfterDiff(loroDoc, pmDoc, mapping, containerId) {
	mapping.clear();
	const innerDoc = containerId ? loroDoc.getContainerById(containerId) : loroDoc.getMap(ROOT_DOC_KEY);
	if (!innerDoc) return;
	mapping.set(innerDoc.id, pmDoc);
	WEAK_NODE_TO_LORO_CONTAINER_MAPPING.set(pmDoc, innerDoc.id);
	const loroChildren = innerDoc.get(CHILDREN_KEY);
	if (!loroChildren) return;
	walkLoroAndPm(loroChildren, pmDoc, mapping);
}
function walkLoroAndPm(loroList, pmNode, mapping) {
	...
	for (let i = 0; i < loroLen && pmIdx < pmChildren.length; i++) {
		...
		if (isLoroText(loroChild)) {
			...
			if (textNodes.length > 0) {
				...
				for (const tn of textNodes) WEAK_NODE_TO_LORO_CONTAINER_MAPPING.set(tn, loroChild.id);
			}
...
const loroUndoPluginKey = new prosemirror_state.PluginKey("loro-undo");
...
function bootstrapDispatch(view, state, docSubscription) {
	...
	const tr = view.state.tr.replace(0, view.state.doc.content.size, new prosemirror_model.Slice(prosemirror_model.Fragment.from(node), 0, 0));
	tr.setMeta(loroSyncPluginKey, {
		type: "update-state",
		state: {
			mapping,
...
function fullReplaceFallback(view, event, state) {
	...
	let tr;
	try {
		const result = diffPmDocs(view.state.tr, view.state.doc, node);
		...
		tr = view.state.tr.replace(0, view.state.doc.content.size, new prosemirror_model.Slice(prosemirror_model.Fragment.from(node), 0, 0));
	...
	tr.setMeta("addToHistory", false);
	view.dispatch(tr);
	rebuildMappingAfterDiff(state.doc, view.state.doc, mapping, state.containerId);
	if (usedFullReplace && anchor != null && !state.disableFallbackCursorRestore) queueMicrotask(() => {
...
var CursorEphemeralStore = class extends loro_crdt.EphemeralStore {
	...
	getLocal() {
		...
		if (!state) return;
		return {
			anchor: state.anchor ? loro_crdt.Cursor.decode(state.anchor) : void 0,
			focus: state.focus ? loro_crdt.Cursor.decode(state.focus) : void 0,
...
const loroEphemeralCursorPluginKey = new prosemirror_state.PluginKey("loro-ephemeral-cursor");
...
var CursorAwareness = class extends loro_crdt.Awareness {
	...
	getLocal() {
		...
		if (!state) return;
		return {
			anchor: state.anchor && loro_crdt.Cursor.decode(state.anchor),
			focus: state.focus && loro_crdt.Cursor.decode(state.focus),
			user: state.user
		};
	}
};
const loroCursorPluginKey = new prosemirror_state.PluginKey("loro-cursor");
...
const LoroUndoPlugin = (props) => {
	const undoManager = props.undoManager || new loro_crdt.UndoManager(props.doc, {});
...
exports.CursorEphemeralStore = CursorEphemeralStore;
exports.LORO_SYNC_META = LORO_SYNC_META;
exports.LoroCursorPlugin = LoroCursorPlugin;
exports.LoroEphemeralCursorPlugin = LoroEphemeralCursorPlugin;
exports.LoroSyncPlugin = LoroSyncPlugin;
exports.LoroUndoPlugin = LoroUndoPlugin;
exports.NODE_NAME_KEY = NODE_NAME_KEY;
exports.ROOT_DOC_KEY = ROOT_DOC_KEY;
exports.absolutePositionToCursor = absolutePositionToCursor;
exports.canRedo = canRedo;
exports.canUndo = canUndo;
exports.convertPmSelectionToCursors = convertPmSelectionToCursors;
exports.createConsoleLogger = createConsoleLogger;
exports.createNodeFromLoroObj = createNodeFromLoroObj;
exports.cursorToAbsolutePosition = cursorToAbsolutePosition;
exports.defaultLogger = defaultLogger;
exports.deltaToPSteps = deltaToPSteps;
exports.diffPmDocs = diffPmDocs;
exports.findContainerLocation = findContainerLocation;
exports.findEmptyTextPosition = findEmptyTextPosition;
exports.getLoroSyncMeta = getLoroSyncMeta;
exports.isLoroInternalTransaction = isLoroInternalTransaction;
exports.loroEventBatchToTransaction = loroEventBatchToTransaction;
exports.loroSyncPluginKey = loroSyncPluginKey;
exports.loroUndoPluginKey = loroUndoPluginKey;
exports.nodeToDelta = nodeToDelta;
exports.rebuildMappingAfterDiff = rebuildMappingAfterDiff;
exports.redo = redo;
