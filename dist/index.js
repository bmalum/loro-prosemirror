...
function applyBlockDiff(tr, oldDoc, newDoc, diff) {
	...
	let newIdx = 0;
	let pos = 0;
	for (const op of diff.children) if (delta.$retainOp.check(op)) for (let i = 0; i < op.retain; i++) {
...
function fullReplaceFallback(view, event, state) {
	...
	try {
		...
		tr = view.state.tr.replace(0, view.state.doc.content.size, new Slice(Fragment.from(node), 0, 0));
